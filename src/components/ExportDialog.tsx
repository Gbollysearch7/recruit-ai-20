...
export function ExportDialog({ isOpen, onClose, items, criteria = [], searchQuery = '' }: ExportDialogProps) {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      switch (format) {
        case 'csv':
          downloadCSV(exportData, `talist-export-${Date.now()}.csv`);
          break;
        case 'json':
          downloadJSON(items, criteria, includeOptions, `talist-export-${Date.now()}.json`, searchQuery);
          break;
        case 'xlsx':
          downloadXLSX(exportData, `talist-export-${Date.now()}.xlsx`);
          break;
      }
