...
const pollResults = async (websetId) => {
  ...
  const handleRefineSearch = async () => {
    ...
    try {
      const result = await createSearch({
        name: searchQuery,
        query: searchQuery,
        count: 10, // Default to 10 for refinement
        criteria,
        enrichments: enrichments.map(e => ({ description: e, format: 'text' }))
      });

      if (result?.id) {
        addToast('New search started', 'success');
        router.push(`/searches/${result.exa_webset_id || result.id}`);
      } else {
        throw new Error('Failed to create search');
      }
    } catch (error) {
      console.error('Refine search error:', error);
      addToast('Failed to start new search', 'error');
    }
  };
  ...
  useEffect(() => {
    ...
    // logic: if not authenticated AND search is not public, redirect
    if (!isAuthenticated && searchShareInfo.isPublic === false) {
      router.push('/');
    }
  }, [isLoading, authLoading, isAuthenticated, searchShareInfo.isPublic, router]);

  if (isLoading && items.length === 0) {
    return <SearchDetailSkeleton />;
  }

  // Strict auth check for non-public searches
  if (!authLoading && !isAuthenticated && searchShareInfo.isPublic === false) {
    return null;
  }
